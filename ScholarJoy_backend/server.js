...
//Groq
const groq = new Groq({ apiKey: "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" }); // 👈 declare it
...
app.post('/rooms', async (req, res) => {
  ...
  if (!subject) {
    subject = new Subject({ name, conversations: [] });
    await subject.save();
  }
...
app.get('/rooms', async (req, res) => {
  const rooms = await Subject.find({}, 'name');
...
app.get('/rooms/:name/messages', async (req, res) => {
  ...
  if (!subject) return res.status(404).json({ message: 'Room not found' });
  res.status(200).json(subject.conversations.sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp)));
});

app.post('/rooms/:name/messages', async (req, res) => {
  const { name } = req.params;
  ...
  subject.conversations.push(newMessage);
  await subject.save();
...
app.post('/generate-tasks', async (req, res) => {
  ...
  try {
    const prompt = `Generate important 5 subtasks for the topic: ${topic}. Provide the output as a JSON object with 'title' as the topic name and 'tasks' as a list of subtask names.`;
    ...
    // Properly remove code block markers
    ...
    const tasks = parsedData.tasks.map(task => ({ name: task, completed: false }));

    const newCourse = new EnrolledCourse({ userId: "", courseName: topic, activities: tasks });
...
app.put('/courses/:courseId', async (req, res) => {
  ...
  try {
    const course = await EnrolledCourse.findByIdAndUpdate(courseId, { activities }, { new: true });
